        ...
        <>
            <div className='pt-10 px-5 sm:px-10 md:px-44 lg:px-60 min-h-screen'>
                <div className='space-y-5 lg:space-y-0 lg:grid grid-cols-3 lg:gap-9'>
                    ...
                    <div>
                        <div>
                            <div className='space-y-3 border padding-5 rounded-md mb-2'>
                                <h1 className='text-primary-color font-medium pb-2 text-center'>Choose Payment Option</h1>
                                <RadioGroup
                                    row
                                    aria-labelledby="demo-row-radio-buttons-group-label"
                                    name="row-radio-buttons-group"
                                    className='flex justify-evenly pb-2'
                                    onChange={handlePaymentChange}
                                    value={paymentGateway}
                                >
                                    {paymentGatwayList.map((item) => <FormControlLabel
                                        className='border w-[35%] rounded-md flex justify-center'
                                        value={item.value}
                                        control={<Radio />}
                                        label={
                                            <img className={`w-14 object-cover`} src={item.image} alt={item.label} />
                                        } />)}
